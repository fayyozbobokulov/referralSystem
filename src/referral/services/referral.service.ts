...
export class ReferralService {
  ...
  async findAllMyReferrals(user: User): Promise<Referral[]> {
    return await this.referralRepository
      .createQueryBuilder('referral')
      .leftJoinAndSelect('referral.user', 'user')
      .leftJoinAndSelect('referral.store', 'store')
  ...
  private async generateReferralLink(referral: Referral): Promise<string> {
    const user = await this.userService.findByPhone(referral.child_phone);
    if (!!user) {
      return `${this.configService.get<string>('HOST')}/api/referral/${
        referral.id
      }`;
    }
    return `${this.configService.get<string>('HOST')}/api/auth/signup`;
  }
}
